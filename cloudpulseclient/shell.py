...
class SecretsHelper(object):
    ...
    def save(self, auth_token, management_url, tenant_id):
        ...
        value = "|".join([str(auth_token),
                          str(management_url),
                          str(tenant_id)])
        keyring.set_password("cloudpulseclient_auth", self._make_key(), value)
    ...
    def management_url(self):
        ...
        management_url = None
        try:
            block = keyring.get_password('cloudpulseclient_auth',
                                         self._make_key())
            if block:
...
class CloudPulseClientArgumentParser(argparse.ArgumentParser):

    def __init__(self, *args, **kwargs):
        super(CloudPulseClientArgumentParser, self).__init__(*args, **kwargs)
...
class OpenStackCloudPulseShell(object):

    def get_base_parser(self):
        parser = CloudPulseClientArgumentParser(
            prog='cloudpulse',
            description=__doc__.strip(),
            epilog='See "cloudpulse help COMMAND" '
...
#            help="Set HTTP call timeout (in seconds)")
        ...
        parser.add_argument('--endpoint-type',
                            ...
                            default=cliutils.env(
                                'CLOUDPULSE_ENDPOINT_TYPE', default=cliutils.env(
                                    'OS_ENDPOINT_TYPE',
        ...
        parser.add_argument('--cloudpulse-api-version',
                            metavar='<cloudpulse-api-ver>',
                            default=cliutils.env(
                                'CLOUDPULSE_API_VERSION',
                                default=DEFAULT_API_VERSION),
                            help='Accepts "api", '
                                 'defaults to env[CLOUDPULSE_API_VERSION].')
        parser.add_argument('--cloudpulse_api_version',
                            help=argparse.SUPPRESS)
    ...
    def main(self, argv):
        ...
        subcommand_parser = (
            self.get_subcommand_parser(options.cloudpulse_api_version)
        )
